...
//printf("Found Room Name %s\n",foundRoom->roomname);                    
                       strcat(buffer, tmpbuf);
                       while(userList != NULL)
                       {
//printf("User Socket %d  - client socket %d\n", userList->socket, client);
...
//printf("Are We Done \n");                 
            ...
            {      
              ...
              pthread_mutex_unlock(&mutex);
              //currentUser->username = arguments[1];
              //username = arguments[1];
              sprintf(buffer, "\nchat>");
              send(client , buffer , strlen(buffer) , 0 ); // send back to client
            } 
            ...
            else if (strcmp(arguments[0], "exit") == 0 || strcmp(arguments[0], "logout") == 0)
            {
                ...
                //. Delete all DMs and current user from DMs list
                //. Delete user groups and current user from Group list
                /*
                dmUser = currentUser->dmhead;
            ...
            // 3. sending a message
                ...
                printf("%s", arguments[0]);
                 ...
                 /*else if (strcmp(arguments[0],"!angry") == 0){ //shouldnt this be arguments[0] because its user input
                   sprintf(tmpbuf,"\n::%s> %s\nchat>", currentUser->username, sbuffer);
                   strcpy(sbuffer, ">:( ");
                 }
                 else if (strcmp(arguments[0],"!feral") == 0){ //shouldnt this be arguments[0] because its user input
                   sprintf(tmpbuf,"\n::%s> %s\nchat>", currentUser->username, sbuffer);
                   strcpy(sbuffer, ">:D");
                 }
                 else if (strcmp(arguments[0],"!happy") == 0){ //shouldnt this be arguments[0] because its user input
                   sprintf(tmpbuf,"\n::%s> %s\nchat>", currentUser->username, sbuffer);
                   strcpy(sbuffer, ":D");
                 }*/
                 else {
...
/* ORIGINAL CODE, courtesy of Izaac 
 * 
 * 
 *   /////////////////////////////////////////////////////
            // 3. sending a message only to members in the same room, or list the client's DM list
            //.    must creat
                 ...
                 // turn chat into the room instead of saying nchat
                 ...
                 while(currentUser != NULL)
                 {
                     ...
                     {  // dont send to yourself 
                         printf("Message From User %s: %s", username,buffer); /// only shows in terminal    //arguments[1]); 
                         //printf(buffer,"\n::%s> %s\nchat>", username,sbuffer);
                         //send(currentUser->socket ,"\n::%s> %s\nchat>", strlen(buffer) , 0 ); // send back to client
